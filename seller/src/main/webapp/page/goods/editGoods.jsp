...
<body class="childrenBody">
	<form class="layui-form" style="width:80%;">
		...
		<div class="layui-form-item">
		    <label class="layui-form-label"><span style="color:red">*</span>价格(元)</label>
		    <div class="layui-input-block">
		    	<input type="text" onkeyup="checknum(this);" id="goodsPrice" value="${goods.goodsPrice }" placeholder="请输入价格" lay-verify="required" class="layui-input">
		    </div>
		</div>
		<div style="margin-top:10px;"></div>
		<div class="layui-inline">
		    ...
		    <div class="goodsImgBox">
				<c:forEach var="item" items="${goods.goodsImgs}">
					<a class="example"><img class="goodsImg" src="<%=basePath%>${item}"></a>
				</c:forEach>
			</div>
	    </div>
	    <div style="margin-top:10px;"></div>
		...
		var form = layui.form,
			...
			$.ajax({
				...
				success: function (data) {
					$('#goodsType').empty();
						for(var p in data){
				            var option = document.createElement("option");  // 创建添加option
				            option.setAttribute("value",data[p].id); // 给option的value添加值
				            option.innerText=data[p].typeName;     
	        ...
	        $("#modifyUser").click(function(){
	        	...
	        	if(goods.goodsType!=''&&goods.goodsTitle!=''&&goods.goodsDesc!=''&&
	        			goods.goodsPrice!=''){
	        		$.ajax({
	            	    	url:"<%=basePath%>modifyGoodsInfo.action",
	            	    	type:"post",
	            	    	data:goods,
	            	    	success:function(data){
